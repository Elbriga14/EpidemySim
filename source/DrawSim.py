...
def drawAllPawns(index, population):
	path = "source/imgs/sim/output" + '{:04d}'.format(index) + ".png"
	plot.figure(1)
	...
	for pawn in population.curedPawnSet:
		curedPawnX.append(pawn.pos.x)
		curedPawnY.append(pawn.pos.y)
	...
	ax.scatter(curedPawnX, curedPawnY, 50, c_ltg, label = "cured")
	ax.legend(fancybox=True, framealpha=0.5, loc="upper right")
	plot.savefig(path, facecolor = c_ltb)
	plot.cla()

def drawSimStatistics(index, totalTime, totalPopulation, infectedValues, healthyValues, sickValues, curedValues):
	path = "source/imgs/stats/output" + '{:04d}'.format(index) + ".png"
	plot.figure(2)
	graph = plot.subplot()
	...
	graph.plot(curedValues, color = c_ltg, linestyle='-', label = "cured")
